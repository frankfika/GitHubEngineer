...
[![Tests: 388 passing](https://img.shields.io/badge/tests-388%20passing-brightgreen.svg)](tests/)
...
The web UI opens ready to work. When you open `http://127.0.0.1:8765/ui/`, it restores the last repository you selected; if that choice is stale, it uses the backend default or the first tracked repository. It then loads the issue inbox immediately:

- **Click a sidebar pill** to switch repositories — the issues endpoint is fetched, the heading switches to "正在读取 X", and on success the inbox fills in.
...
When you don't want to live in a terminal, `ghe --serve` starts a local-first maintainer-assistant UI on `127.0.0.1:8765` (override with `GHE_SERVE_PORT` or `--serve-host`). Open it in any browser; the layout adapts to desktop and mobile and follows the system light/dark appearance. You can generate briefs, configure the coding agent, review isolated repairs, and—only after all safety gates pass—explicitly create a Draft PR.
...
| `/api/briefs/generate` | `POST` | Start generating a brief for a tracked repository. |
| `/api/brief-jobs/<id>` | `GET` | Read queued/running/completed brief-generation status. |
...
- **No separate login layer.** It binds to `127.0.0.1` by default and rejects non-loopback or cross-origin state-changing requests; only use `--serve-host 0.0.0.0` when you intentionally want to expose the health surface on the LAN.
- **Local writes are explicit.** Decisions, tracked repositories, coding-agent settings, and repair workspaces are stored below the configured `.ghe/` or desktop application-data directory.
- **GitHub writes are gated.** Repair work happens in an isolated local workspace. Creating a fork, branch, or PR requires a verified repair, an authenticated account, and a one-use confirmation token generated by the final publish click. The review surface is the full diff — hunk-level decisions were removed in the v1.0 UX revision.
- **No background daemon.** Run it under `systemd`, `tmux`, or a process manager for long-running use.

**Repository loading state machine.** The main panel never shows a misleading "正在读取 X" with a stuck spinner. Concretely:

- On startup the remembered/default repository is selected and `GET /api/repositories/<repo>/issues` runs. With no tracked repositories, the heading remains **「未选择仓库」** and the add-repository empty state is shown.
- If a fetch fails, the heading rolls back to **「X · 读取失败」** in a warning color and the inbox shows an error card with a 「重试一次」 button. The five-minute foreground refresh will also retry while the page remains open.
...
#   green "我的" / amber "外部" badges, first repository selected and loading
```
...
| `ghe --serve [--serve-host 127.0.0.1]` | Start the local desktop/web service. Brief generation and repair actions are explicit. |
...
No. Brief analysis is read-only against GitHub and writes local files (`reports/*.md`, `.ghe/history/*.json`, `tasks/*.md`) plus the optional `$GITHUB_STEP_SUMMARY`. Explicit UI actions may also update tracked repositories, coding-agent configuration, and decision memory locally. Automatic repair stays in an isolated workspace; the only GitHub write is an explicitly confirmed PR after the verification gate has passed and the maintainer has reviewed the full diff.
...
**Q: Which repository opens when the app starts?**
The UI restores the last repository you selected. If that repository is no longer tracked, it opens the backend default (or the first tracked repository) and immediately loads its issues. A truly fresh install with no repositories shows the add-repository onboarding instead.

**Q: How do I preview the UI without a `GITHUB_TOKEN`?**
Run `GHE_MOCK_REPOSITORIES=1 ghe --serve --serve-host 127.0.0.1`. The mock only covers `/api/repositories` (returns two fixed repos, one owner + one monitor), so the sidebar, owner/monitor badges, default selection, the success path on a public repo, and the failure-recovery flow all work. Issue fetches and the `+ 添加仓库` POST still hit the real API; private repos and write actions still need a token.
...
| --- | --- | --- | --- |
| **Owner (完整模式)** | The viewing user owns the repo *and* has both `gh auth` and `claude auth` | Edit in an isolated worktree, run tests, review the full diff, create a **Draft PR** directly on the source repo | Source repo (as a Draft PR) |
| **Fork (Fork 模式)** | The repo is external *and* `gh auth` + `claude auth` are both set | Edit inside the viewer's own fork, run tests, review the full diff, open a PR against the original repo via the fork | The viewer's fork, then upstream via PR |
| **Anonymous (匿名模式)** | The repo is public and the viewer has at least `claude auth` (no `gh` required) | Browse issues, clone the public repo, run Claude Code, review the full diff; **no outbound writes** | `~/.githubengineer/repos/<owner>/<repo>/<issue#>/` on the viewer's machine |
...
4. **Review** — the **CodeMirror 6 diff view** (see §D) opens automatically on `review_ready`, showing the full patch with a verification badge, the agent's one-line change summary, and a stats line (files / adds / removes). The user reads the diff end-to-end; there are no per-hunk accept/reject decisions in the v1.0 UX.
...
The diff view is the review surface for the full-patch "review and submit" flow. It opens **automatically** inside `#repair-inspector` once a job reaches `review_ready` — the inspector dialog is **not** closed, so the user keeps the conversation history (and the new **修复过程** live-progress timeline) visible while reading the patch.
...
- Closed by the inspector's *关闭* button; reopening requires the user to reopen the same `review_ready` job from the sidebar.

### D.2 Layout (top → bottom)

- **Top metadata bar** — repository, issue number, files / adds / removes, verification badge.
- **Verification + summary panel** (max-height 156 px, scrollable) — the same one-line verification status surfaced in the inspector, plus the agent's one-line change summary (`job.agent_summary`).
- **Code heading + single-column unified diff** — adds / removes / context, with a 200 KB bundle (vs. Monaco's 4.5 MB, ~20× lighter).
- **Status line + publish CTA** — the bottom strip is the only place that drives the publish action; the button label tracks the verification state (`修复未通过验证` / `等待验证` / `提交修复` / `连接 GitHub 后提交` / `演示任务不可提交`).
...
3. On a successful connection, a toast appears: **已升级到完整模式 · 准备提交**. The diff panel and the live-progress timeline stay mounted; nothing in the existing review state is discarded.
4. The `job.hunk_decisions` map still exists in the data model and is written server-side when the user clicks the publish button (it serves as the pre-submit integrity check), but the UI no longer exposes per-hunk buttons. The hunk-decision endpoint is therefore an implementation detail of the full-patch submit path, not a user-facing affordance.
