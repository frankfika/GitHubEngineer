...
- **GitHub writes are gated.** Repair work happens in an isolated local workspace. Creating a fork, branch, or Draft PR requires a verified repair, a completed hunk review, an authenticated account, and a one-use confirmation token generated by the final publish click.
